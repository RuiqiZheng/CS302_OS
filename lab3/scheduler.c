...
struct waitqueue* jobselect() {
    ...
    if (head) {
        for (prev = head, p = head; p != NULL; prev = p, p = p->next) {
            ...
            } else if (p->job->curpri == highest) // when pri eq, choose one whose wait_time is higher
            {
                if (p->job->wait_time > select->job->wait_time) {
                    select = p;
                    selectprev = prev;
                }
            }
...
void do_stat()
{
	...
	*/
	...
	struct waitqueue *p;
	char timebuf[BUFLEN];
	//Modify do_stat() in scheduler.c such that it can also output jobname, current
    //priority and default priority
	printf( "JID\tPID\tOWNER\tRUNTIME\tWAITTIME\tCREATTIME\tJNAME\tCURPRI\tDEFPRI\tSTATE\n");
...
int main()
{
    ...
    // tv.usec is in Microseconds....10-6
	interval.tv_sec = 0;
